...
description: 檢閱上的組態設定 [!UICONTROL Advanced] &gt； [!UICONTROL Admin] 商務管理員頁面。
...
| [!UICONTROL Forgot and Reset Email Sender] | 全域 | 識別顯示為「 」寄件者的商店聯絡人 _忘記密碼_ 電子郵件。 預設寄件者： `General Contact`<br/>其他寄件者選項： `Sales Representative`， `Customer Support`， `Custom Email` |
...
如需有關變更這些設定的詳細資訊，請參閱 [變更啟動頁面](../../getting-started/admin-dashboard.md#change-the-startup-page) 在 _快速入門手冊_.
...
| `Sales` | `Operations` | [`Quotes`](../../b2b/quotes.md) ![適用於Adobe Commerce的B2B](../../assets/b2b.svg) <br/>[`Orders`](../../stores-purchase/orders.md)<br/>[`Invoices`](../../stores-purchase/invoices.md)<br/>[`Shipments`](../../stores-purchase/shipments.md)<br/>[`Credit Memos`](../../stores-purchase/credit-memos.md)<br/>[`Billing Agreements`](../../stores-purchase/paypal-billing-agreements.md)<br/>[`Returns`](../../stores-purchase/returns.md) ![Adobe Commerce](../../assets/adobe-logo.svg) </span><br/>[`Transactions`](../../stores-purchase/transactions.md)<br/>`Braintree Virtual Terminal` |
| `Catalog` | [`Inventory`](../../inventory-management/introduction.md) | [`Products`](../../catalog/products-list.md)<br/>[`Categories`](../../catalog/categories.md)<br/>[`Shared Catalog`](../../b2b/catalog-shared-create.md) ![適用於Adobe Commerce的B2B](../../assets/b2b.svg) |
| `Customers` | [`All Customers`](../../customers/customers-all.md)<br/>[`Now Online`](../../customers/now-online.md)<br/>[`Customer Groups`](../../customers/customer-groups.md)<br/>[`Segments`](../../customers/customer-segments.md) ![Adobe Commerce](../../assets/adobe-logo.svg) <br/>[`Companies`](../../b2b/account-companies.md)![適用於Adobe Commerce的B2B](../../assets/b2b.svg) |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
如需有關設定這些選項的詳細資訊，請參閱 [設定基底URL](../../stores-purchase/store-urls.md#configure-the-base-url) 在 _商店和購買體驗指南_.

| 欄位 | [範圍](../../getting-started/websites-stores-views.md#scope-settings) | 說明 |
|------------------------------------|------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [!UICONTROL Use Custom Admin URL] | 全域 | 決定是否使用自訂URL來存取管理員。 選項： `Yes` / `No` |
| [!UICONTROL Custom Admin URL] | 全域 | 指定存取管理員的自訂URL。 依預設，管理員URL與基底URL相同。<br/>**重要：** 管理員URL必須位於相同的Commerce安裝中，並與店面具有相同的檔案根目錄。 |
...
如需有關設定這些選項的詳細資訊，請參閱 [設定管理員安全性](../../systems/security-admin.md) 在 _Admin System指南_.

| 欄位 | [範圍](../../getting-started/websites-stores-views.md#scope-settings) | 說明 |
|--------------------------------------------------------|------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [!UICONTROL Admin Account Sharing] | 存放區檢視 | 決定管理員使用者是否可從不同裝置同時登入相同帳戶。 選項： <br/>**`Yes`**— 允許來自相同管理員帳戶的多個作用中工作階段。<br/>**`No`**  — 每個管理員帳戶只允許一個作用中的工作階段。 |
| [!UICONTROL Password Reset Protection Type] | 存放區檢視 | 決定用來管理密碼重設要求的方法。 選項： <br/>**`By IP and Email`**— 收到通知的回應後，可將密碼重設至與Admin帳戶關聯的電子郵件地址。<br/>**`By IP`**  — 密碼可線上上重設，不需額外確認。 <br/>**`By Email`**— 密碼只能透過電子郵件回應傳送至與Admin帳戶相關之電子郵件地址的通知來重設。<br/>**`None`**  — 密碼只能由存放區管理員重設。 |
...
| [!UICONTROL Password Change] | 全域 | 決定是否要求管理員使用者變更密碼。 選項： <br/>**`Forced`**— 要求管理員使用者在設定帳戶後變更密碼。<br/>**`Recommended`**  — 建議管理員使用者在設定帳戶後變更密碼。 |
...
如需有關設定這些選項的詳細資訊，請參閱 [管理員儀表板](../../getting-started/admin-dashboard.md) 在 _快速入門手冊_.
...
如需有關設定這些選項的詳細資訊，請參閱 [限制產品顯示](../../catalog/products-list.md#limit-product-display) 在 _目錄管理指南_.
...
如需有關設定這些選項的詳細資訊，請參閱 [驗證碼](../../systems/security-captcha.md) 在 _Admin System指南_.

| 欄位 | [範圍](../../getting-started/websites-stores-views.md#scope-settings) | 說明 |
|-------------------------------------------------------|------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [!UICONTROL Enable CAPTCHA in Admin] | 全域 | 啟用驗證碼以供管理員登入。 選項： `Yes` / `No` |
| [!UICONTROL Font] | 全域 | 決定用於顯示驗證碼的字型。 若要新增您自己的字型，請將字型檔案放在與Commerce執行個體相同的目錄中，並將宣告新增至config.xml檔案，網址為 `app/code/Magento/Captcha/etc` 預設字型：` LinLibertine` |
| [!UICONTROL Forms] | 全域 | 決定使用CAPTCHA的表單。 選項： `Admin Login` / `Admin Forgot Password` |
| [!UICONTROL Displaying Mode] | 全域 | 決定驗證碼何時出現。 選項： <br/>**`Always`**— 必須具備驗證碼才能登入。<br/>**`After number of attempts to login`**  — 顯示 [!UICONTROL Number of Unsuccessful Attempts to Login] 欄位。 輸入允許的登入嘗試次數。 值0 （零）類似於將「顯示模式」設定為「一律顯示」。 此選項不會涵蓋「忘記密碼」和「建立使用者」表單。 如果已啟用驗證碼並設定為顯示，則會始終將其包含在表單中。<br />**注意**：若要追蹤失敗的登入嘗試次數，系統會將每次嘗試使用單一電子郵件地址登入及使用一個IP位址登入計算。 允許來自相同IP位址的登入嘗試次數上限為1,000。 此限制僅在啟用驗證碼時適用。 |
...
![管理員動作記錄](./assets/admin-actions-logging.png)<!-- zoom -->

如需有關設定這些選項的詳細資訊，請參閱 [動作記錄存檔](../../systems/action-log-archive.md) 在 _Admin System指南_.
...
![管理員使用情況](./assets/admin-usage.png)<!-- zoom -->

如需有關設定這些選項的詳細資訊，請參閱 [使用資料集合](../../getting-started/admin.md#usage-data-collection) 在 _快速入門手冊_.

| 欄位 | 範圍 | 說明 |
|------------------------------------------|--------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [!UICONTROL Enable Admin Usage Tracking] | 全域 | 授予Adobe收集管理員使用資料的許可權，以改善使用體驗 _管理員_&#x200B;以及相關的產品和服務。 允許資料收集也會啟用 _產品內指引_ 旨在將說明、工具提示、逐步指南、入門資訊、功能公告等互動式內容帶至 _管理員_. 使用資料中未識別個別管理員。 選項：<br />**`Yes`**— 允許資料收集並啟用 _產品內指引_.<br />**`No`**  — 不允許資料收集或啟用 _產品內指引_. |
