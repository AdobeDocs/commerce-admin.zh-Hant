...
description: 檢閱上的組態設定 [!UICONTROL Advanced] &gt； [!UICONTROL Developer] 商務管理員頁面。
...
>[!NOTE]
>
>這些組態設定可在 [開發人員模式](../../systems/developer-tools.md#operation-modes) 僅限。

## [!UICONTROL Frontend Development Workflow]

![前端開發工作流程](./assets/developer-frontend-development-workflow.png)<!-- zoom -->

如需有關變更這些設定的詳細資訊，請參閱 [前端開發工作流程](../../systems/developer-tools.md#frontend-development-workflow) 在 _Admin System指南_.

| 欄位 | [範圍](../../getting-started/websites-stores-views.md#scope-settings) | 說明 |
|--- |--- |--- |
| [!UICONTROL Workflow Type] | 全域 | 決定開發期間在使用者端或伺服器端是否發生較少編譯。 選項： <br/>**`Client side less compilation`**— 使用原生less.js程式庫在瀏覽器中編譯。<br/>**`Server side less compilation`**  — 使用Less PHP程式庫在伺服器上進行編譯。 這是生產的預設模式。 |
...
如需有關變更此設定的詳細資訊，請參閱 [使用者端限制](../../systems/developer-tools.md#client-restrictions) 在 _Admin System指南_.

| 欄位 | [範圍](../../getting-started/websites-stores-views.md#scope-settings) | 說明 |
|--- |--- |--- |
| [!UICONTROL Allow IPs (comma separated)] | 存放區檢視 | 建立IP位址允許清單，這些IP位址可以在即時網站上使用開發人員工具，而不會干擾商店中的客戶。 使用開發者工具時網站的任何變更，例如 _內嵌翻譯_，則只會從允許清單上的IP位址中可見。 |
...
如需有關變更這些設定的詳細資訊，請參閱 [正在最佳化資源檔案](../../systems/developer-tools.md#optimizing-resource-files) 在 _Admin System指南_.

| 欄位 | [範圍](../../getting-started/websites-stores-views.md#scope-settings) | 說明 |
|--- |--- |--- |
| [!UICONTROL Allow Symlinks] | 存放區檢視 | 正在啟用 [符號連結](https://en.wikipedia.org/wiki/Symbolic_link) 可能會讓您的網站面臨安全性風險，不建議用於生產存放區。 |
...
如需有關變更這些設定的詳細資訊，請參閱 [範本路徑提示](../../systems/developer-tools.md#template-path-hints) 在 _Admin System指南_.
...
如需有關變更這些設定的詳細資訊，請參閱 [翻譯內嵌](../../systems/developer-tools.md#translate-inline) 在 _Admin System指南_.
...
| [!UICONTROL Enable JavaScript Bundling] | 存放區檢視 | 決定多個JavaScript檔案是否可以整合到一個檔案中。 選項： `Yes` / `No` |
...
| [!UICONTROL Translation Strategy] | 全域 | 決定系統使用的翻譯方法。 選項： <br/>**`Dictionary`**— 店面側翻譯。<br/>**`Embedded`**  — 管理員端翻譯。 |
...
| [!UICONTROL Use CSS critical path] | 全域 | 此 _CSS關鍵路徑_ 提供縮制的關鍵CSS內嵌於 `<head>` 和會遞延非同步載入的所有非關鍵樣式。 選項： `Yes` / `No` |
...
| 欄位 | [範圍](../../getting-started/websites-stores-views.md#scope-settings) | 說明 |
|--- |--- |--- |
| [!UICONTROL Asynchronous Indexing|Global] | 決定訂單、商業發票、出貨及銷退折讓單等訂單處理系統實體何時加入網格並重新編制索引。 非同步索引可用於避免在儲存操作期間鎖定資料，並縮短處理時間。 選項： <br/>**`Disable`**- （預設）與訂單相關的實體會在不同時間加入格線。 在儲存時顯示。<br/>**`Enable`**  — 訂單相關實體僅在排程的cron作業期間新增至網格。 Cron應設定為每分鐘執行一次。 |
