...
description: 檢閱Commerce管理員的[!UICONTROL Sales] &gt； [!UICONTROL Delivery Methods]頁面上的組態設定。
...
| 包含稅捐目標金額 | 網站 | 決定稅捐是否包含在「最小訂單金額」計算中。 選項： <br/>**是** — 計算最小訂單金額時包含稅捐（小計+稅捐 — 折扣）。<br/>**否** — 計算最小訂單金額時不包含稅捐（小計 — 折扣）。 |
...
| [!UICONTROL Mode] | 網站 | 決定傳送至UPS系統之資料所使用的傳輸模式。 選項： <br/>**`Development`**- UPS不會驗證從Commerce伺服器收到的資料是否透過SSL傳送。<br/>**`Live`** - UPS會驗證從Commerce伺服器收到的資料是否透過安全通訊端層(SSL)傳送。 |
...
| [!UICONTROL Calculate Handling Fee] | 網站 | 設定表格費率出貨的處理費計算方法。 選項： <br>**`Fixed`**— 手續費是固定費率。<br>**`Percent`** — 處理費是以訂單金額的百分比來套用。 |
...
![USPS帳戶設定](./assets/delivery-methods-usps.png)<!-- zoom -->
...
| [!UICONTROL USPS Type] | 網站 | 選擇&#x200B;**USPS Rest API**。 不支援&#x200B;**USPS Web Tools API**。 |
| [!UICONTROL User ID] | 網站 | 您的USPS託運人帳戶使用者識別碼。 |
| [!UICONTROL Password] | 網站 | 您的USPS託運人帳戶密碼。 |
| [!UICONTROL Mode] | 網站 | 決定傳送至USPS系統之資料所使用的傳輸模式。 選項包括： <br/>**`Development`**- USPS不會驗證從Commerce伺服器收到的資料是否透過SSL傳送。<br/>**`Live`** - USPS會驗證從Commerce伺服器收到的資料是否透過安全通訊端層(SSL)傳送。 |
...
| [!UICONTROL Account Number] | 網站 | 您的USPS **帳號**，用於REST API的付款。<br/><br/>**_Note:_**&#x200B;此欄位是選擇性的；不過，必須使用此欄位才能建立送貨標籤。 |
| [!UICONTROL Customer Registration Identifier(CRID)] | 網站 | 客戶註冊識別碼(CRID)是USPS產生的數值代碼，可唯一識別REST API所在位置的企業。<br/><br/>**_Note:_**&#x200B;此欄位為選用，但必須啟用送貨標籤建立。 |
| [!UICONTROL Mailer Identifier(MID)] | 網站 | 郵件識別碼(MID)是用於識別郵件的「智慧型郵件」條碼中的欄位。 USPS會將MID指派給要求提供REST API的「郵件擁有者」、「郵件代理程式」或其他服務提供者。<br/><br/>**_Note:_**&#x200B;此欄位是選擇性的；不過，必須使用此欄位才能建立送貨標籤。 |
| [!UICONTROL Manifest MID] | 網站 | 指定給REST API資訊清單的唯一郵件程式識別碼。<br/><br/>**_Note:_**&#x200B;此欄位是選擇性的；不過，必須啟用送貨標籤建立。 |
...
如果未顯示下列欄位，您必須套用[USPS REST API移轉品質修補程式](https://experienceleague.adobe.com/zh-hant/docs/commerce-operations/tools/quality-patches-tool/patches-available-in-qpt/v1-1-70/ac-15210)。 此修補程式可支援USPS API，這是REST型平台，可取代Web Tools API。 如需詳細資訊，請參閱[USPS Web Tools API淘汰](../../stores-purchase/carriers.md)。
...
| [!UICONTROL Customer Registration Identifier(CRID)] | 網站 | 客戶註冊識別碼(CRID)是USPS產生的數值代碼，可唯一識別REST API所在位置的企業。<br/><br/>**_Note:_**&#x200B;此欄位為選用，但必須啟用送貨標籤建立。 |
| [!UICONTROL Mailer Identifier(MID)] | 網站 | 郵件識別碼(MID)是用於識別郵件的「智慧型郵件」條碼中的欄位。 USPS會將MID指派給要求提供REST API的「郵件擁有者」、「郵件代理程式」或其他服務提供者。<br/><br/>**_Note:_**&#x200B;此欄位是選擇性的；不過，必須使用此欄位才能建立送貨標籤。 |
| [!UICONTROL Manifest MID] | 網站 | 指定給REST API資訊清單的唯一郵件程式識別碼。<br/><br/>**_Note:_**&#x200B;此欄位是選擇性的；不過，必須啟用送貨標籤建立。 針對套用[AC-15210](https://experienceleague.adobe.com/zh-hant/docs/commerce-operations/tools/quality-patches-tool/patches-available-in-qpt/v1-1-70/ac-15210)修補程式的Magento 2.4.7-p8，[!UICONTROL Manifest MID]為必填欄位。 |
| [!UICONTROL AES/ITN] | 網站 | USPS AES - Automated Export System / ITN - REST API的內部交易編號。 <br/><br/>**_Note:_**&#x200B;此欄位通常是選用的，但在下列情況下若要啟用送貨標籤建立，此欄位是必要的： <ul><li>出貨中的每一種貨品（由排程B的匯出代碼<a href="https://www.census.gov/foreign-trade/schedules/b" target="_blank">www.census.gov/foreign-trade/schedules/b</a>定義）價值在$2,500或以下，不需要匯出授權；或</li><li>不論價值為何，這批貨物都會被送往加拿大，不需要出口許可證。</li></ul> |
...
| [!UICONTROL Calculate Handling Fee] | 網站 | 設定表格費率出貨的處理費計算方法。 選項： <br/>**`Fixed`**— 手續費是固定費率。<br/>**`Percent`** — 處理費是以訂單金額的百分比來套用。 |
| [!UICONTROL Handling Applied] | 網站 | 指定處理費是套用至每個訂單，還是套用至訂單中的每個套件。 |
| [!UICONTROL Handling Fee] | 網站 | 設定包含在運費價格中的處理。 處理費可設為固定金額或百分比。 <br/><br/>**_注意:_**&#x200B;輸入百分比量時，請使用25%的小數位數格式`25`。 |
...
| [!UICONTROL Free Shipping Amount Threshold] | 網站 | 決定免費送貨是否需要最低訂單金額。 選項： <br/>**`Enable`**— 針對符合最小金額的訂單，啟用免費FedEx出貨。<br/>**`Disable`** — 停用FedEx免運費最低訂單。 |
...
| [!UICONTROL DHL Type] | 網站 | 選擇REST或XML，視您要使用的專案而定。 |
| [!UICONTROL Access ID] | 網站 | 您的DHL託運人帳戶存取識別碼。 |
| [!UICONTROL API KEY] | 網站 | 您用於REST API的DHL API金鑰。 |
| [!UICONTROL Password] | 網站 | 您的DHL託運人帳戶密碼。 |
| [!UICONTROL API Secret] | 網站 | 您的REST API DHL API秘密金鑰。 |
...
| 手續費 | 網站 | 根據您選擇用來計算金額的方法，輸入處理費的金額。 例如，如果費用是以固定費用為基礎，則以小數點輸入金額，例如`4.90`。 不過，如果處理費是以訂單的百分比為基準，請以百分比輸入金額。 例如，如果您要收取訂單的6%費用，請輸入值為`6`。 |
...
| [!UICONTROL Sandbox Mode] | 網站 | 若使用沙箱，則設為`Yes`。 針對即時模式設定為`No`。 |
