...
![平準率](./assets/delivery-methods-flat-rate.png)<!-- zoom -->
...
![表格費率](./assets/delivery-methods-table-rates.png)<!-- zoom -->
...
| [!UICONTROL Enable Negotiated Rates] | 網站 | （僅限UPS XML）根據您與UPS的合約啟用/停用特殊費率。 選項： `Yes` / `No` |
...
| [!UICONTROL Shipper Number] | 網站 | （僅限UPS XML）若要參考使用議定匯率，必須有6個字元的「UPS出貨人編號」。 |
| [!UICONTROL Container] | 網站 | 設定用來包裝訂單出貨的貨櫃型態。 選項： `Customer Packaging` / `UPS Letter Envelope` / `Customer Packaging` / `UPS Letter Envelope` / `UPS Tube` / `UPS Express Box` / `UPS Worldwide 25 kilo` / `UPS Worldwide 10 kilo` |
| [!UICONTROL Weight Unit] | 網站 | 設定商店中產品重量的預設測量單位。 另請參閱 [維度權數](../../stores-purchase/carriers.md#dimensional-weight) 以取得其他資訊。 |
| [!UICONTROL Tracking XML URL] | 網站 | （僅限UPS XML）用來追蹤封裝的UPS URL。 |
| [!UICONTROL Destination Type] | 網站 | 設定預設出貨目的地型態。 選項： `Business` / `Residential` |
| [!UICONTROL Maximum Package Weight] | 網站 | 設定UPS所指定之封裝的最大重量。 如果訂購的產品超過最大包裝重量，則無法使用此送貨選項。 根據 [UPS.com](https://www.ups.com/us/en/global.page)，包裝不得超過150磅（70公斤）請洽詢您的運送業者，以確認最大重量。 |
...
| [!UICONTROL Free Method] | 網站 | 識別透過UPS用於免費送貨方法的方法。 若要停用免費送貨，請選擇[無]。 <br/><br/>**_注意：_**此方法類似於「基本」 [免運費](../../stores-purchase/shipping-free.md)，但在結帳時會顯示為UPS送貨選項。 |
...
| [!UICONTROL Ship to Applicable Countries] | 網站 | 指定允許哪些國家/地區客戶使用此送貨方法。 選項： <br/>**`All Allowed Countries`**— 來自所有客戶的客戶 [國家/地區](../../getting-started/store-details.md#country-options) 在商店設定中指定的可使用此送貨方法。<br/>**`Specific Countries`**  — 選擇此選項後， [!UICONTROL Ship to Specific Countries] 清單隨即顯示。 選取清單中可使用此送貨方法的每個國家/地區。 |
| [!UICONTROL Show Method if Not Applicable] | 網站 | 決定UPS在出庫時是否一律顯示為送貨選項。 選項： <br/>**`Yes`**— 結帳時，UPS一律會顯示為送貨選項，即使不適用於該訂單亦然。<br/>**`No`**  — 只有適用於訂單時，UPS才會於結帳時顯示為送貨選項。 （例如，訂單重量超過最大重量金額）。 |
...
![FedEx帳戶設定](./assets/delivery-methods-fedex-account-settings.png)<!-- zoom -->
...
| 手續費 | 網站 | 根據您選擇用來計算金額的方法，輸入處理費的金額。 例如，如果費用是以固定費用為基礎，則以小數點輸入金額，例如 `4.90`. 不過，如果處理費是以訂單的百分比為基準，請以百分比輸入金額。 例如，如果您對訂單的6%計費，則輸入值為 `.06`. |
...
| [!UICONTROL Ready Time] | 網站 | 指定提交訂單後，包裝何時準備好取貨（以小時為單位）。 |
...
| [!UICONTROL Ship to Applicable Countries] | 網站 | 指定允許哪些國家/地區客戶使用此送貨方法。 選項： <br/>**所有允許的國家/地區**  — 所有允許的國家/地區均適用免費送貨方法。 允許的國家/地區在 [!UICONTROL General] 設定頁面。 <br/>**特定國家/地區**  — 將此運送選項限製為「送貨至特定國家/地區」清單中指定的國家/地區。 |
| [!UICONTROL Ship to Specific Countries] | 網站 | 指定可以傳送DHL出貨的國家/地區。 若有下列情況，則會使用此選取的國家清單 `Specific Countries` 已選取的 [!UICONTROL Ship to Applicable Countries] 選項。 |
| [!UICONTROL Show Method if Not Applicable] | 網站 | 決定DHL在結帳期間何時顯示為送貨方法。 選項： <br/>**`Yes`**— 結帳時，DHL會一律顯示為送貨選項，即使不適用於該訂單。<br/>**`No`**  — 只有在訂單適用時（即訂單重量超過最大重量金額），DHL才會在結帳時顯示為送貨選項。 |
