...
>[!IMPORTANT]
>
>在設定Google reCAPTCHA之前，您必須確定 `PHP.ini` 檔案包含下列設定： `allow_url_fopen = 1`. 這可能需要開發人員協助。 另請參閱 [PHP設定](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/prerequisites/php-settings.html) 在 _安裝指南_.

{{config}}

如需使用Google reCAPTCHA來保護商店安全的詳細資訊，請參閱Google [reCAPTCHA](../../systems/security-google-recaptcha.md) 在 _Admin System指南_.
...
| 欄位 | [範圍](../../getting-started/websites-stores-views.md#scope-settings) | 說明 |
|--|--|--|
| [!UICONTROL Enable for Customer Login] | 網站 | 指定客戶使用時的reCAPTCHA型別 [登入](../../customers/customer-sign-in.md) 至其帳戶。 選項：<br/>**`No`**- （預設）不會驗證登入要求。<br />**`reCAPTCHA v2 ("I am not a robot")`**  — 要求使用者選取 _我不是機器人_ 核取方塊。<br />**`Invisible reCAPTCHA v2`**— 在背景驗證使用者行為，而不需要根據分數互動。<br/>**`Invisible reCAPTCHA v3`** - （建議）根據互動分數，在背景驗證使用者行為。 |
...
| [!UICONTROL Enable for Gift Card] | 網站 | ![Adobe Commerce](../../assets/adobe-logo.svg) (僅限Adobe Commerce)指定客戶輸入 [禮品卡](../../catalog/product-gift-card-create.md) 程式碼。 選項：<br/>**`No`**- （預設）不會驗證禮卡代碼提交。<br />**`reCAPTCHA v2 ("I am not a robot")`**  — 要求使用者選取 _我不是機器人_ 核取方塊。<br />**`Invisible reCAPTCHA v2`**— 在背景驗證使用者行為，而不需要根據分數互動。<br/>**`Invisible reCAPTCHA v3`** - （建議）根據互動分數，在背景驗證使用者行為。 |
| [!UICONTROL Enable for Invitation Create Account] | 網站 | 指定客戶傳送建立帳戶時所使用的reCAPTCHA型別 [邀請](../../merchandising-promotions/invitations.md) 程式碼。 選項：<br/>**`No`**- （預設）不會驗證邀請電子郵件提交。<br />**`reCAPTCHA v2 ("I am not a robot")`**  — 要求使用者選取 _我不是機器人_ 核取方塊。<br />**`Invisible reCAPTCHA v2`**— 在背景驗證使用者行為，而不需要根據分數互動。<br/>**`Invisible reCAPTCHA v3`** - （建議）根據互動分數，在背景驗證使用者行為。 |
| [!UICONTROL Enable for Send to Friend] | 網站 | 指定客戶使用時的reCAPTCHA型別 [共用產品](../../stores-purchase/email-a-friend.md) 和朋友在一起。 選項：<br/>**`No`**- （預設）不會驗證電子郵件提交。<br />**`reCAPTCHA v2 ("I am not a robot")`**  — 要求使用者選取 _我不是機器人_ 核取方塊。<br />**`Invisible reCAPTCHA v2`**— 在背景驗證使用者行為，而不需要根據分數互動。<br/>**`Invisible reCAPTCHA v3`** - （建議）根據互動分數，在背景驗證使用者行為。 |
| [!UICONTROL Enable for Wishlist Sharing] | 網站 | 指定客戶使用時的reCAPTCHA型別 [共用願望清單](../../stores-purchase/wishlist-storefront.md#share-the-wish-list). 選項：<br/>**`No`**- （預設）不會驗證訊息和電子郵件提交。<br />**`reCAPTCHA v2 ("I am not a robot")`**  — 要求使用者選取 _我不是機器人_ 核取方塊。<br />**`Invisible reCAPTCHA v2`**— 在背景驗證使用者行為，而不需要根據分數互動。<br/>**`Invisible reCAPTCHA v3`** - （建議）根據互動分數，在背景驗證使用者行為。 |
