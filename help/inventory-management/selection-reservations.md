...
SSA會使用庫存和來源，檢查傳入產品請求的銷售管道，並決定可用的庫存：
...
當您處理和出貨訂單時，演演算法會有所幫助。 您可以執行演演算法，以取得運送產品的最佳來源建議，或覆寫選項至：
...
若要設定、指派及訂購自訂存貨的來源，請參閱[排定存貨來源的優先順序](stocks-prioritize-sources.md)。
...
距離優先順序演演算法會比較出貨目的地地址的地點與來源地點，以決定最接近完成出貨的來源。 這個距離可能取決於實際距離或從一個位置到另一個位置旅行的時間，使用匯入的資料庫位置或Google方向（駕駛、步行或騎腳踏車）。

您有兩個選項可以計算距離和時間，以找出最接近出貨履行情況的來源：

- [!UICONTROL Google MAP] — 使用[Google Maps Platform](https://cloud.google.com/maps-platform/)服務來計算送貨目的地地址與來源位置（地址與GPS座標）之間的距離與時間。 此選項會使用來源的經緯度。 必須有Google API金鑰，並啟用[地理編碼API](https://developers.google.com/maps/documentation/geocoding/start)和[距離矩陣API](https://developers.google.com/maps/documentation/distance-matrix/start)。 此選項需要Google計費計畫，並可能透過Google產生費用。

- [!UICONTROL Offline Calculation] — 使用下載和匯入的地理碼資料來計算距離，以判斷最接近出貨目的地地址的來源。 此選項使用送貨地址與來源的國家/地區代碼。 若要設定此選項，您可能需要開發人員協助，才能使用命令列先下載及匯入地理代碼。
...
- **未完成的預留**。 系統會總計尚未補償的所有初始預留。 此數字一律為負數。 如果客戶A有十個料號的預訂，而客戶B有五個料號的預訂，則產品總計的未結預訂–15。
...
中繼資料`object_type`必須是`order`，而`object_id`是訂單識別碼。
...
1. 客戶傳送了20個專案的商業發票，基本上取消了5個訂購單位。
...
- *已提交訂單* — 當針對數個產品提交訂單時，會輸入該數量的預訂。 例如，從美國網站訂購五個揹包，會針對該SKU和存貨輸入`-5`的預訂。 可銷售數量會減少5。
