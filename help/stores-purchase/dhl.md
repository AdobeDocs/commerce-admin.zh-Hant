...
1. 如果您使用DHL REST API，請將&#x200B;**[!UICONTROL DHL Type]**&#x200B;設為`DHL REST`。

   如果您使用DHL XML API，請將&#x200B;**[!UICONTROL DHL Type]**&#x200B;設為`DHL XML`。

   >[!NOTE]
   >
   >DHL REST API是與DHL整合的偏好方法。 此XML API已過時，在未來版本中可能會移除。

1. 使用DHL提供的憑證完成下列欄位：

如果您使用DHL REST API，則必須提供下列認證：

    - **[!UICONTROL API KEY]**
    - **[!UICONTROL API SECRET]**

如果您使用DHL XML API，您必須提供下列認證：
...
1. 根據您的要求設定手續費選項。
   ...
   - 針對&#x200B;**[!UICONTROL Handling Fee]**，根據您選擇用來計算金額的方法，輸入要收取的金額。
     ...
     例如，如果費用是以固定費用為基礎，則以小數點輸入金額，例如`4.90`。 不過，如果處理費是以訂單的百分比為基準，請以百分比輸入金額。 例如，如果您要收取訂單的6%費用，請輸入值為`6`。
...
1. DHL提供&#x200B;**[!UICONTROL sandbox mode]**&#x200B;選項。 如果您使用沙箱模式，請將&#x200B;**[!UICONTROL sandbox mode]**&#x200B;設為`Yes`。
如果您使用即時模式，請將&#x200B;**[!UICONTROL sandbox mode]**&#x200B;設為`No`。

   >[!NOTE]
   >
   >沙箱模式僅用於測試目的。 它可讓您測試與DHL的整合，而不會影響您的即時商店。
