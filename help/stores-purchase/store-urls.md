...
Adobe Commerce或Magento Open Source安裝中的每個網站都有一個指派給店面的基本URL，以及另一個指派給管理員的URL。 Adobe會使用變數來定義與基底URL相關的內部連結，如此一來，您就可以在不更新連結的情況下，將整個商店從一個位置移到另一個位置。 標準基底URL以`http`開頭，安全基底URL以`https`開頭。
