...
| 欄位 | 說明 |
|--- |--- |
| [!UICONTROL Type] | 指定套件的型別。 選取其中一個預先定義的值。 可用的封裝型別因各貨運業者而異。 當「建立封裝」彈出式視窗開啟時，出貨承運商的預設封裝會顯示在「型別」欄位中。 如果您選取並非由出貨承運商設計的封裝，則必須輸入封裝的尺寸。 對於為DHL、FedEx和UPS出貨建立的出貨標籤，「貨品型別」欄位設定為`Merchandise`。 對於USPS，欄位會反映&#x200B;_視窗中_&#x200B;內容&#x200B;_[!UICONTROL Create Packages]_&#x200B;欄位的值。 |
