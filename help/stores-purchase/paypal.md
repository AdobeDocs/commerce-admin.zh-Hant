...
若要在商店提供PayPal付款方式，您必須擁有PayPal [企業帳戶](https://manager.paypal.com/)和/或[PayPal Payflow帳戶](https://developer.paypal.com/docs/payflow/payflow-gateway/)。 帳戶需求會在每個PayPal解決方案的說明中指定。 您的PayPal商家帳戶也可用來管理任何[欺詐篩選器](#paypal-fraud-management-filters)，這些篩選器套用到從您的商店進行的購買。
...
PayPal PayLater讓您的客戶能夠快速取得融資，因此他們現在就可以購買產品，並隨著時間付費，您無需支付額外費用。 客戶選擇PayPal信用額度選項時不會向您收費，您只需支付一般的PayPal交易費用。 若要深入瞭解，請參閱[PayPal網站](https://www.paypal.com/us/business/buy-now-pay-later)。
...
對於美國商家，預設會針對[PayPal Express結帳](paypal-express-checkout.md)付款選項啟用PayPal信用額度。 若要針對此付款方式停用它，請參閱&#x200B;_PayPal Express結帳設定_&#x200B;的[功能](paypal-express-checkout.md#features)區段。
