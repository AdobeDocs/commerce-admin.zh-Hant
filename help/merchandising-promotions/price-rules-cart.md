...
| [!UICONTROL Start] | ![Magento Open Source](../assets/open-source.svg) （僅限Magento Open Source）使用動態行事曆欄位（_[!UICONTROL To:]_和_[!UICONTROL From:]_）根據建立規則時定義的規則開始日期來篩選清單。 |
| [!UICONTROL End] | ![Magento Open Source](../assets/open-source.svg) （僅限Magento Open Source）使用動態行事曆欄位（_[!UICONTROL To:]_和_[!UICONTROL From:]_），根據建立規則時定義的規則結束日期來篩選清單。 |
