...
- 所有排定的更新都會連續套用。 這表示任何實體一次只能有一個排程更新。

- 任何排定的更新都會套用至其時間範圍內的所有存放區檢視。 因此，一個實體無法同時擁有不同存放區檢視的不同排程更新。 所有存放區檢視中的所有實體屬性值（不受目前排程更新影響）都是從預設值取得，而不是從先前的排程更新取得。

- 如果相同促銷活動中執行了多個價格規則，則價格規則的「優先順序」設定會決定哪一個規則優先。 若要深入瞭解，請參閱[內容暫存](../content-design/content-staging.md)。

## 在特定時間結束價格規則銷售

如果有效價格規則是在沒有結束日期的情況下建立的，且您必須在特定時間結束，則無法編輯現有的排定更新以新增結束日期。 您必須建立新的排程更新，以將規則狀態變更為`Inactive`。 將此新更新的開始日期設定為您希望銷售結束的日期和時間。
...
>[!NOTE]
>
>如果行銷活動連結到多個目錄價格規則，您只能從[內容測試儀表板](../content-design/content-staging-dashboard.md)編輯行銷活動。
...
1. 若要返回行事曆，請按一下預覽頁面標頭中的&#x200B;**[!UICONTROL Calendar]**。
   ...
   **分享預覽連結**
   ...
   若要與其他管理員使用者共用商店預覽的連結，請按一下&#x200B;**[!UICONTROL Share]**。 將連結複製到剪貼簿，然後貼到電子郵件內文中。

   >[!NOTE]
   >
   >如果您的[角色具有管理管理員使用者帳戶的存取權](../systems/permissions-user-roles.md)，您可以建立或更新具有管理員許可權的現有使用者帳戶，以便共用預覽連結。
