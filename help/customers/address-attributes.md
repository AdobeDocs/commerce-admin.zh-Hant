...
1. 在&#x200B;**[!UICONTROL Attribute Properties]**&#x200B;區段中，執行下列動作：
   ...
   - 輸入識別系統內屬性的&#x200B;**[!UICONTROL Attribute Code]**。
     ...
     >[!TIP]
     >
     >**_Shortcut:_**&#x200B;若要只完成必要欄位，請向下捲動至[!UICONTROL Storefront Properties]，輸入[!UICONTROL Sort Order]，然後儲存。
...
1. 若要在儲存記錄之前檢查輸入到欄位中的資料是否準確，請將&#x200B;**[!UICONTROL Input Validation]**&#x200B;設定為允許在該欄位中的資料型別。 可用的值視指定的&#x200B;_[!UICONTROL Input Type]_而定。
   ...
   >[!NOTE]
   >
   >對於系統定義的客戶地址屬性（例如&#x200B;_電話_、_城市_&#x200B;和&#x200B;_街道_），無論&#x200B;**[!UICONTROL Input Validation]**&#x200B;設定為何，Commerce都會強制內建伺服器端驗證。 這些預設規則會限制每個欄位允許的字元（例如，電話號碼只能包含數字、空格和特定符號）。 **[!UICONTROL Input Validation]**&#x200B;設定會新增更多限制，但無法覆寫內建驗證，無法透過Admin UI停用。
