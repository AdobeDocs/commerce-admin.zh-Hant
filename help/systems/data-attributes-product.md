...
下表列出典型產品匯出的屬性，其顯示順序為預設順序。 每個屬性在CSV檔案中以欄的形式呈現，而產品記錄則以列呈現。 以底線開頭的欄包含服務資料，例如複雜資料的屬性或選項值。 您可以 [匯出](data-export.md) 目錄中的產品，用來檢視資料中每個屬性的呈現方式。

用於匯出此資料的安裝已安裝範例資料，並有兩個網站和多個商店檢視。 雖然此清單包含通常會匯出的所有欄，但 `sku` 是唯一必要的值。 若要匯入資料，您只能包含含有變更的欄。 此 `sku` 應該是第一欄，但其餘屬性的順序並不重要。
...
| `product_type` | 表示產品型別。 值：<br/>`simple`  — 通常以單一單位或固定數量出售的有形專案。<br/>`grouped`  — 一組單獨產品，作為一組產品銷售。<br/>`configurable`  — 一種產品，包含客戶在購買之前必須選取的多個選項。 您可以為每組變數管理詳細目錄，因為它們代表具有不同SKU的獨立產品。 例如，可設定產品的顏色和大小組合與目錄中的特定SKU相關聯。<br/>`virtual`  — 一種不需要出貨且不存放在存貨中的無形產品。 範例包括服務、成員資格和訂閱。<br/>`bundle`  — 可自訂的產品集，包含一起銷售的簡單產品。 |
...
| product_online | 判斷產品是否可在市集內銷售。 值：<br/>`1` — （是）產品已啟用，且可供銷售。<br/>`2` — （否）產品已停用，且無法銷售。 |
| `tax_class_name` | 與此產品相關聯的稅捐類別名稱。 |
| `visibility` | 決定產品是否顯示在目錄中，以及是否可供搜尋。 值：<br/>`Not Visible Individually`  — 產品不包含在產品清單中，儘管它可能是其他產品的變體。<br/>`Catalog`  — 產品會出現在所有目錄清單中。<br/>`Search`  — 產品可用於搜尋操作。<br/>`Catalog, Search`  — 產品包含在目錄清單中，也可供搜尋。 |
| `price` | 產品在您的商店中出售的價格。 |
...
| save_rewrites_history | 當提供值時 `1` 有新的 `url_key`，會產生新的301 URL重新寫入，以便將舊URL重新導向至新URL。 |
...
| `base_image` | 產品頁面上主要影像的相對路徑。 Commerce會以字母資料夾結構在內部儲存檔案。 您可以在匯出的資料中檢視每個影像的確切位置。 例如： `/sample_data/m/b/mb01-blue-0.jpg`<br/>若要上傳新影像或覆寫現有影像，請輸入檔案名稱，其前面加上正斜線。 例如： `/image.jpg` |
...
| `page_layout` | 決定產品頁面的頁面配置。 值：<br/>`No layout updates`  — 不變更頁面配置。<br/>`1 column`  — 將單欄版面配置套用至產品頁面。<br/>`2 columns with left bar`  — 將具左側欄的雙欄版面配置套用至產品頁面。<br/>`2 columns with right bar`  — 將帶有右側欄的雙欄版面配置套用至產品頁面。<br/>`3 columns`  — 將三欄版面配置套用至產品頁面。<br/>`empty`  — 將空白版面配置套用至產品頁面。 |
| `product_options_container` | 如果產品有多個選項，可決定這些選項在產品頁面上出現的位置。 值：產品資訊欄/資訊欄後的區塊 |
| `msrp_display_actual_price_type` | 決定客戶看到產品實際價格的位置。 值：<br/>`In Cart`  — 顯示購物車中的實際產品價格。<br/>`Before Order Confirmation`  — 在確認訂單之前，於結帳程式結束時顯示實際產品價格。<br/>`On Gesture`  — 當客戶按一下 _按一下以取得價格_ 或 _這是什麼？_ 連結。 |
...
| `use_config_min_qty` | 決定是否使用組態中的預設值，且與「使用組態設定」核取方塊相對應。 值：<br/>`1` — （是）此屬性的值會使用預設組態設定。<br/>`0` （或空白） — （否）可覆寫此屬性的值的預設組態。 |
...
| `notify_on_stock_below` | 指定觸發 _無庫存_ 通知。 |
| `use_config_notify_stock_qty` | 決定預設組態設定是否用於觸發庫存層級通知，且對應至「使用組態設定」核取方塊的狀態。 值：<br/>`1` — （是）<br/>`0` （或空白） — （否） |
| `manage_stock` | 決定是否使用存貨控制來管理產品。 值：<br/>`1` — （是）啟用完整存貨控制來管理產品的存貨水準。<br/>`0` （或空白） — （否）系統不會追蹤目前庫存中的專案數。 |
...
| `crosssell_position` | 決定在SKU中列為「交叉銷售產品」的SKU位置（排序順序）。 `crosssell_skus` 欄。 |
| `upsell_skus` | 列出已識別為向上銷售之每項產品的SKU。 |
| `upsell_position` | 決定在SKU中列為追加銷售產品的SKU位置（排序順序）。 `upsell_skus` 欄。 |
...
| `_associated_sku` | 與分組產品相關聯之產品的SKU。 | 已分組的產品 <br/>套件組合產品 |
| `_associated_default_qty` | 決定包含的相關產品的數量。 | 可設定的產品<br/>已分組的產品<br/>套件組合產品 |
| `_associated_position` | 與其他相關產品一起列出時，決定相關產品的位置。 | 可設定的產品<br/>已分組的產品<br/>套件組合產品 |
...
![可設定的產品 — 匯出的資料](./assets/data-exported-configurable-product.png){width="600" zoomable="yes"}
...
「進階價格匯入/匯出」可讓您快速更新產品群組與階層價格的定價資訊。 要執行的程式 [匯入](data-import.md) 和 [匯出](data-export.md) 進階價格資料與任何其他實體型別相同。 範例CSV檔案包含支援進階定價之每種產品型別的層級與群組價格。 變更進階價格不會影響其餘的產品記錄。
...
|--- |--- |
| `sku` | （必要）「庫存單位」是用於追蹤存貨的唯一英數字元識別碼。 SKU的長度最多可為64個字元。 例如： `sku123`<br/>**_注意：_**超過64個字元的SKU會導致匯入失敗。 |
| `tier_price_website` | 此 [網站程式碼](../stores-purchase/stores.md#add-websites) 會識別每個可使用層級定價的網站。 例如： `-  website1 -  All Websites [USD]` |
| `tier_price_customer` | 識別 [客戶群組](../customers/customer-groups.md) 提供層級定價的地方。 例如： `-  ALL GROUPS -  NOT LOGGED IN -  General -  Wholesale -  Retailer` |
...
| `tier_price` | 產品的折扣層級價格。 的 [套件組合產品](../catalog/product-create-bundle.md)，層級價格會以百分比計算。 |
| `group_price_website` | 此 [網站程式碼](../stores-purchase/stores.md#add-websites) 每個有群組定價的網站的使用者。 如果指定多個網站，請以逗號分隔每個網站，但不含空格。 例如： `-  website1 -  All Websites [USD]` |
| `group_price_customer_group` | 識別可使用群組定價的客戶群組。 例如： `-  NOT LOGGED IN -  General -  Wholesale -  Retailer` |
| `group_price` | 產品的折扣群組價格。 的 [套件組合產品](../catalog/product-create-bundle.md)，群組價格會以百分比計算。 |
