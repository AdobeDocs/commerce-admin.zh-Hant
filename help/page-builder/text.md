---
title: 元素 — 文字
description: 瞭解文字內容型別，用於在 [!DNL Page Builder] 階段。
...
使用 _文字_ 內容型別，以使用WYSIWYG （「您所見即所得」）編輯器新增文字容器，在 [[!DNL Page Builder] 階段](workspace.md#stage). 此外，您還可以新增連結、影像、 [變數](../systems/variables-predefined.md)，並從編輯器工具列將Widget移至文字。

![橫幅上的格式化文字](./assets/pb-storefont-banner-with-button.png){width="700"}
...
| 複製 | ![「複製」圖示](./assets/pb-icon-duplicate.png){width="25"} | 製作文字容器的副本。 |
...
1. 在 [!DNL Page Builder] 面板，展開 **[!UICONTROL Elements]** 並拖曳 **[!UICONTROL Text]** 舞台上的列、欄或標籤集的預留位置。
...
1. 視需要使用編輯器來輸入文字並設定其格式。
   ...
   ![包含內容的文字編輯器](./assets/pb-elements-text-editor.png){width="600"}
...
1. 選取編輯器空間中的文字，然後按一下 _插入/編輯連結_ ( ![插入/編輯連結按鈕](./assets/pb-icon-add-link.png){width="20"} )。
...
1. 按一下 _插入/編輯影像_ ( ![插入/編輯影像按鈕](./assets/icon-pb-add-image.png){width="20"} )。

1. 的 **[!UICONTROL Source]**，按一下搜尋圖示，使用媒體儲存空間來尋找和選取影像。

1. 的 **[!UICONTROL Image Description]**，輸入影像的描述文字。
...
1. 若要插入影像，然後返回 [!DNL Page Builder] 工作區，按一下 **[!UICONTROL OK]**.
...
1. 更新 _[!UICONTROL Advanced]_設定。
   ...
   - 設定 **[!UICONTROL Border]** 套用至文字容器所有四個邊的樣式：
   ...
   - 以畫素為單位，輸入 **[!UICONTROL Margins and Padding]** 以決定文字容器的外邊界和內邊距。
