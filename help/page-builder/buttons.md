---
title: 元素 — 按鈕
description: 瞭解Buttons內容型別，用於在 [!DNL Page Builder] 階段。
...
使用 _按鈕_ 內容型別，以新增中的個別按鈕或一組按鈕 [[!DNL Page Builder] 階段](workspace.md#stage). 您可以水平或垂直排列按鈕，並將它們直接新增到舞台上的列、欄、標籤和橫幅中。

![店面有按鈕的橫幅](./assets/pb-storefont-banner-with-button.png){width="600" zoomable="yes"}

{{$include /help/_includes/page-builder-save-timeout.md}}

## 工具箱

當您使用「按鈕」內容型別時，您可以新增和編輯個別按鈕以及容納一或多個按鈕的按鈕容器。 每個都有自己的工具箱，您可用來設計 [!DNL Page Builder] 階段。
...
| 複製 | ![「複製」圖示](./assets/pb-icon-duplicate.png){width="25"} | 製作按鈕的副本。 |
...
| 新增 | ![「新增」圖示](./assets/pb-icon-add-button.png){width="25"} | 將按鈕新增至容器。 |
| （標籤） | 按鈕 | 將目前的容器識別為按鈕元素。 |
...
| 複製 | ![「複製」圖示](./assets/pb-icon-duplicate.png){width="25"} | 製作按鈕容器的副本。 |
...
1. 在 [!DNL Page Builder] 面板，展開 **[!UICONTROL Elements]** 並拖曳 **[!UICONTROL Buttons]** 舞台上的列、欄或標籤集的預留位置。
...
1. 設定 **[!UICONTROL Button Link]** 使用下列其中一種型別：
   ...
   - **[!UICONTROL Product]**  — 輸入產品名稱（部分或完整）或SKU，然後在清單中選擇產品名稱。
     ...
     >[!NOTE]
     >
     >產品會根據 _顯示無庫存產品_ 設定。 針對多來源商家，使用 [Inventory management](../inventory-management/introduction.md)，產品清單僅受指派給預設網站的來源限制。
...
1. 完成 [進階設定][advanced-settings] 視需要。
...
1. 將滑鼠指標暫留在按鈕容器上以顯示工具箱，然後選擇 _新增_ ( ![「新增」圖示](./assets/pb-icon-add-button.png){width="20"} )圖示。

   ![新增另一個按鈕](./assets/pb-elements-buttons-toolbox-add.png){width="500" zoomable="yes"}

1. 輸入要顯示在第二個按鈕上的文字。

1. 按一下新按鈕以顯示其工具箱，然後選擇 _設定_ ( ![設定圖示](./assets/pb-icon-settings.png){width="20"} )圖示。

   ![編輯按鈕](./assets/pb-elements-button-set-edit-button2-toolbox.png){width="500" zoomable="yes"}

1. 設定 **[!UICONTROL Button Type]** 至 `Secondary`.

1. 設定 **[!UICONTROL Button Link]** 視需要。

   在以下範例中，連結是前往的相對URL [聯絡我們](../getting-started/store-details.md#contact-us-form) 頁面。
...
1. 再次按一下舞台上的第二個按鈕，然後選擇 _複製_ ( ![「複製」圖示](./assets/pb-icon-duplicate.png){width="20"} )圖示。
...
1. 按一下第三個按鈕以顯示工具箱，然後選擇 _設定_ ( ![設定圖示](./assets/pb-icon-settings.png){width="20"} )圖示。
   ...
   ![第三個按鈕的工具箱](./assets/pb-elements-button-set-find-us-toolbox-settings.png){width="500" zoomable="yes"}

1. 更新 **[!UICONTROL Button Link]** 視需要。

1. 在右上角，按一下 **[!UICONTROL Save]** 以套用設定並返回 [!DNL Page Builder] 工作區。

### 步驟3：更新按鈕容器

1. 將滑鼠指標暫留在按鈕容器上以顯示工具箱，然後選擇 _設定_ ( ![設定圖示](./assets/pb-icon-settings.png){width="20"} )圖示。

   ![按鈕容器工具箱](./assets/pb-elements-buttons-toolbox-settings.png){width="500" zoomable="yes"}

1. 在 _[!UICONTROL Appearance]_，選擇&#x200B;**[!UICONTROL Stacked]**.

1. 設定 **[!UICONTROL All Buttons are same size]** 至 `Yes`.

   ![相同大小的棧疊按鈕](./assets/pb-elements-buttons-settings-appearance-stacked.png){width="300"}

1. 視需要更新其餘的設定，使用中的說明 [變更按鈕容器的設定][button-container].

1. 完成後，按一下 **[!UICONTROL Save]** 以套用設定並返回 [!DNL Page Builder] 工作區。

   完整的棧疊按鈕組出現在舞台上，包含一個主要按鈕和兩個次要按鈕。

   ![舞台上的棧疊按鈕](./assets/pb-elements-buttons-stacked.png){width="500" zoomable="yes"}

## 移動按鈕

1. 按一下您要移動的按鈕。

1. 選取並拖曳移動( ![移動圖示](./assets/pb-icon-move.png){width="20"} )圖示（顯示在按鈕文字之前）至按鈕容器內按鈕的新位置。
...
1. 視需要更新標準設定。
   ...
   - **[!UICONTROL Button Link]**  — 決定按一下按鈕時提供的目標頁面。
...
1. 若要儲存設定並返回 [!DNL Page Builder] 工作區，按一下 **[!UICONTROL Save]** 位於右上角。

## 變更按鈕容器的設定

1. 將滑鼠指標暫留在按鈕容器上以顯示工具箱，然後選擇 _設定_ ( ![設定圖示](./assets/pb-icon-settings.png){width="20"} )圖示。

1. 更新 **[!UICONTROL Appearance]** 設定。
   ...
   - 設定 **[!UICONTROL All buttons are same size]** 根據您的偏好設定選擇選項。
...
您可以修改 _[!UICONTROL Advanced]_個別按鈕和按鈕容器的設定。
...
1. 設定 **[!UICONTROL Border]** 套用至按鈕或按鈕容器全部四邊的樣式：
...
1. （選擇性）指定下列專案的名稱： **[!UICONTROL CSS classes]** 從目前樣式表中套用至按鈕或按鈕容器。

   以空格分隔多個類別名稱。

1. 以畫素為單位，輸入 **[!UICONTROL Margins and Padding]** 決定按鈕或按鈕容器的外部邊界和內邊距。
