...
您的[!DNL Commerce]帳戶包含您可以提供給受信任員工和協助管理您網站的服務提供者的資訊。 當您共用帳戶存取權時，所有敏感資訊（例如您的帳單歷史記錄或信用卡資訊）都會受到保護，不會提供給其他使用者。

主要帳戶持有人有權授予其他[!DNL Commerce]帳戶持有人的有限存取權。 共用的存取權可以撤銷，但無法傳輸。 對於``Cloud Shared Access from MAG[XYZ]``專案，無法在此處&#x200B;**刪除使用者記錄**，但存取&#x200B;**仍可撤銷**。

只有具有適當許可權的主要帳戶持有人才能正式授予共用存取權。 如果主要帳戶持有人不再擁有存取權或已離開公司，客戶應使用[Commerce帳戶轉移程式](/help/getting-started/commerce-account-transfer.md)將擁有權轉移給新連絡人。 雖然Commerce支援團隊在有限的情況下可以模擬客戶，但客戶應設定共用存取權，以降低安全性和責任風險。


![共用存取設定](./assets/shared-access.png){width="600" zoomable="yes"}

「帳單歷史記錄」區段只會顯示在帳單系統變更前建立的舊發票。 如果您沒有看到任何較新的商業發票，則表示這些商業發票已轉換為新系統，無法從此檢視表存取。
...
1. 開始之前，請先從&#x200B;**新共用存取權受權者**&#x200B;的[!DNL Commerce]帳戶取得下列資訊：
   ...
   - 使用者必須已在account.adobe.com註冊帳戶，並透過account.magento.com登入。 如需詳細資訊，請參閱[建立Commerce帳戶](/help/getting-started/commerce-account-create.md#create-a-commerce-account)。
...
>[!NOTE]
>
>不需要共用對&#x200B;_[!UICONTROL Security Tool]_&#x200B;的存取權 — 任何具有MAGE ID的使用者都可以使用自己的帳戶設定安全性掃描工具。 他們只需要必要的許可權，就能變更網站，並使用[必要的方法](/help/systems/security-scan.md)之一來驗證網域的所有權。
...
1. 若要將共用帳戶連線到說明中心，請按一下共用帳戶左側導覽面板中的&#x200B;**[!UICONTROL Support]**。
   ...
   >[!NOTE]
   >
   >收到共用存取許可權後，若要[在Experience League上提交支援案例](https://experienceleague.adobe.com/en/docs/support-resources/adobe-support-tools-guide/adobe-commerce-support/adobe-commerce-help-center-user-guide#support-case)，請務必先在左欄中選取以「([!DNL Commerce])」結尾的組織名稱。
...
>[!NOTE]
>
>您無法在此介面中從MAG[XYZ ]_刪除共用名稱為_&#x200B;雲端共用存取的使用者。 請參閱[如何刪除透過雲端專案被授予共用存取許可權的使用者？](https://experienceleague.adobe.com/en/docs/support-resources/adobe-support-tools-guide/adobe-commerce-support/adobe-commerce-help-center-user-guide#remove-cloud-shared-access-users)。
