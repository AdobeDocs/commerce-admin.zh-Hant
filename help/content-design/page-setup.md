...
「HTML標題」區段中的設定與 `<head>` HTML標籤，並可針對每個商店檢視進行設定。 除了頁面標題、說明和關鍵字的中繼資料之外，區段還包含指向Favicon的連結，以及其他指令碼。 搜尋引擎自動機制的指示以及商店示範通知的顯示也在本節中設定。
...
1. 在 _其他設定_，展開 ![展開選擇器](../assets/icon-display-expand.png) 此 **[!UICONTROL HTML Head]** 區段。

   ![HTML標頭組態設定](./assets/configuration-html-head.png){width="500" zoomable="yes"}

1. 更新 [Favicon](../getting-started/storefront-branding.md#add-a-favicon) 如有需要。
...
1. 輸入任何 **[!UICONTROL Scripts and Style Sheets]** 視需要。

1. 啟用或停用 [示範商店通知](../getting-started/storefront-branding.md#set-the-store-demo-notice) 如有需要。
...
| [!UICONTROL Scripts and Style Sheets] | 存放區檢視 | 包含必須在結束前包含在HTML中的指令碼 `<head>` 標籤之間。 例如，必須放在之後的任何第三方JavaScript `<body>` 可在此輸入標籤。 |
...
![標題組態設定](./assets/configuration-header.png){width="400" zoomable="yes"}
...
1. 在 _其他設定_，展開 ![展開選擇器](../assets/icon-display-expand.png) 此 **[!UICONTROL Header]** 區段。
...
| [!UICONTROL Translate Title] | 存放區檢視 | 決定 `Page Title` 或 `Meta Title` 應該要翻譯。 |

{style="table-layout:auto"}

## 頁尾

頁尾設定區段是您更新 [版權宣告](../getting-started/storefront-branding.md#change-the-copyright-notice) ，然後輸入必須在結尾之前放置的其他命令檔 `<body>` 標籤之間。
...
1. 進行必要的變更 **[!UICONTROL Copyright]** 和 **[!UICONTROL Miscellaneous HTML]** 設定。
...
| 欄位 | 範圍 | 說明 |
|--- |--- |--- |
| [!UICONTROL Miscellaneous HTML] | 存放區檢視 | 一個輸入方塊，您可在其中將其他指令碼上傳到伺服器，這些指令碼必須放在結尾之前 `<body>` 標籤之間。 |
| [!UICONTROL Copyright] | 存放區檢視 | 顯示在每頁底部的版權宣告。 若要包含版權符號，請使用HTML字元實體 `\&copy;` 如下所示： `\&copy; 2021 Commerce Demo Store. All Rights Reserved.` 請務必將範例版權宣告取代為您自己的版權宣告。 |
